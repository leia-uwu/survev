...
const paramsSchema = z.object({
    ...
    mapId: z.string().min(1).transform((v) => Number(v)),
    maxCount: z.number(),
    ...
    teamMode: z.enum(["solo", "duo", "squad"])
      .catch("solo")
...
function getLowestScoreCacheKey({ teamMode, mapId, type }: {teamMode: TeamMode, mapId: number, type: string}) {
 return `leaderboard:lowest:${teamMode}:${mapId}:${type}`;
}
function getLeaderboardCacheKey({ teamMode, mapId, type }: {teamMode: TeamMode, mapId: number, type: string}) {
