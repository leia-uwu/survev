...
import { type Image, createCanvas } from "canvas";
...
export async function createSpritesheets(
    ...
    const { images, trimmedImages } = await scaleImages(paths);
    ...
    const high = createSheet(2);
    const low = "in development lol" ? high : createSheet(0.5);
    return {
        low,
        high
    };
