...
import { MsgStream, MsgType, Msg } from "../../shared/net";
...
export class Game {
    ...
    tryJoinGame(url: string, matchPriv: string, loadoutPriv: string, questPriv: string, onConnectFail: () => void) {
        if (
            !this.connecting &&
            !this.connected &&
            !this.initialized
    ...
    init() {
        ...
        // @ts-expect-error ShotBarn doesn't accept any arguments ~~
        ...
        for (const type in TypeToPool) {
            if (TypeToPool.hasOwnProperty(type)) {
                this.objectCreator.registerType(type, TypeToPool[type as unknown as keyof typeof TypeToPool]);
            }
        }
        // Render ordering
    ...
    update(dt: number) {
        ...
        if (!this.spectating) {
            ...
            for (let i = 0; i < this.ui2Manager.uiEvents.length; i++) {
                ...
                if (e.action == "use") {
                    if (e.type == "weapon") {
                        ...
                        const input = weapIdxToInput[e.data as unknown as keyof typeof weapIdxToInput];
                        if (input) {
                            inputMsg.addInput(input);
                        }
                    } else {
            ...
            if (this.uiManager.roleSelected) {
                ...
                this.sendMessage(MsgType.PerkModeRoleSelect, roleSelectMessage, 128);
        ...
        if (specBegin || (this.spectating && specNext) || specPrev) {
            ...
            this.sendMessage(MsgType.Spectate, specMsg, 128);
        }
        ...
        for (const k in inputMsg) {
            if (inputMsg.hasOwnProperty(k)) {
                ...
                } else if (this.prevInputMsg[k as keyof typeof this.prevInputMsg] != inputMsg[k as keyof typeof inputMsg]) {
                    diff = true;
                }
                if (diff) {
                    break;
        ...
        if (diff || this.inputMsgTimeout < 0) {
            ...
            this.sendMessage(MsgType.Input, inputMsg, 128);
        ...
        for (let i = 0; i < this.emoteBarn.newPings.length; i++) {
            ...
            msg.pos = ping.pos;
            msg.isPing = true;
            this.sendMessage(MsgType.Emote, msg, 128);
        }
    ...
    processGameUpdate(msg: UpdateMsg) {
        const ctx: Ctx = {
        ...
        this.activePlayer = this.playerBarn.getPlayerById(this.activeId) as Player;
    ...
    sendMessage(type: MsgType, data: Msg, maxLen: number) {
