import { WoodsSpring } from './../../shared/defs/maps/woodsSpringDefs';
import { Emote } from './../../server/src/objects/player';
import $ from "jquery";
import * as PIXI from "pixi.js-legacy";
import { GameObjectDefs } from "../../shared/defs/gameObjectDefs";
import { EmoteDef, EmotesDefs } from "../../shared/defs/gameObjects/emoteDefs";
import { PingDef, PingDefs } from "../../shared/defs/gameObjects/pingDefs";
import { GameConfig, EmoteSlot } from "../../shared/gameConfig";
...
import { AmmoDef, GunDef, LootDef, MeleeDef } from '../../shared/defs/objectsTypings';
...
export class EmoteBarn {
    ...
    newPings: { type: string; pos: Vec2; }[] = [];
    newEmotes: { type: string; pos: Vec2; }[] = [];
    emoteLoadout: string[] = [];
    unlockTypes = {};
    ...
    constructor(
        ...
        // Set ping wheel specific data
        const teamPingData:  Record<string, {
            parent: JQuery<HTMLElement>,
            vA: Vec2,
            vC: Vec2,
            ping: string, 
        ...
        for (const key in teamPingData) {
            if (teamPingData.hasOwnProperty(key)) {
                const pingData = teamPingData[key as keyof typeof teamPingData ];
    ...
    addPing(ping: Emote, factionMode: boolean) {
        ...
        if (this.activePlayer) {
            const pingData = PingDefs[ping.type] as PingDef;
            if (pingData) {
    ...
    addEmote(emote: Emote) {
        const emoteData = EmotesDefs[emote.type] as EmoteDef;
        if (emoteData) {
            ...
            if (emote.type == "emote_loot") {
                const lootDef = GameObjectDefs[emote.itemType] as GunDef | MeleeDef;
                if (lootDef?.lootImg) {
    ...
    update(dt: number, localId: number, player: Player, teamMode: number, deadBodyBarn: DeadBodyBarn, map: Map, renderer: Renderer, input: InputHandler, inputBinds: InputBinds, spectating: boolean) {
