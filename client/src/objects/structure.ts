...
import { StructureDef } from "../../../shared/defs/mapObjectsTyping";
import { AABB, Collider, coldet } from "../../../shared/utils/coldet";
...
export class Structure implements AbstractObject {
    ...
    layers!: {
        objId: number,
        collision: Collider & { height: number },
        underground: boolean
    }[];
    stairs!: Partial<StructureDef["stairs"]>;

    mask!: (AABB & { height: number})[]
    ...
    updateData(data: ObjectData<ObjectType.Structure>, fullUpdate: boolean, isNew: boolean, ctx: Ctx) {
        if (fullUpdate) {
            this.type = data.type;
            this.layer = 0;
