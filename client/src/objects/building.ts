import { FloorImage } from './../../../REMOVE_ME/BuildingDefs';
...
import { BuildingDef, DecalDef } from "../../../shared/defs/mapObjectsTyping";
...
export class Building implements AbstractObject {
    ...
    soundEmitters: Array<{ instance: SoundHandle | null; sound: string; channel: string; pos: Vec2; range: {min: number, max: number}; falloff: number; volume: number }> = [];
    ...
    imgs: { sprite: PIXI.Sprite; isCeiling: boolean; removeOnDamaged?: boolean; zOrd: number; zIdx: number; }[] = []
    ...
    updateData(data: ObjectData<ObjectType.Building>, fullUpdate: boolean, isNew: boolean, ctx: Ctx) {
        if (fullUpdate) {
            this.type = data.type;
    ...
    update(dt: number, map: Map, particleBarn: ParticleBarn, audioManager: AudioManager, ambience: unknown, activePlayer: Player, renderer: Renderer, camera: Camera) {
        // Puzzle effects
        if (this.hasPuzzle) {
            const def = MapObjectDefs[this.type] as BuildingDef;
    ...
    positionSprite(sprite: PIXI.Sprite, alpha: number, camera: Camera) {
