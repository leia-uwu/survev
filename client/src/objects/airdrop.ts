...
class AirDrop implements AbstractObject {
    ...
    updateData(data: ObjectData<ObjectType.Airdrop>, fullUpdate: boolean, isNew: boolean, ctx: Ctx) {
        if (isNew) {
            this.isNew = true;
...
export class AirdropBarn {
    ...
    update(dt: number, activePlayer: Player, camera: Camera, map: Map, particleBarn: ParticleBarn, renderer: Renderer, audioManager: AudioManager) {
        const airdrops = this.airdropPool.getPool();
        for (let i = 0; i < airdrops.length; i++) {
