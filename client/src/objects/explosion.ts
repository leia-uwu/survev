...
import { Map } from "../map";
import { Particle, ParticleBarn } from "./particles";
...
class Explosion {
    ...
    update(dt: number, explosionBarn: ExplosionBarn, particleBarn: ParticleBarn, audioManager: AudioManager, map: Map, camera: Camera) {
...
export class ExplosionBarn {
    ...
    update(dt: number, map: Map, playerBarn: PlayerBarn, camera: Camera, particleBarn: ParticleBarn, audioManager: AudioManager, debug: unknown) {
        for (let i = 0; i < this.explosions.length; i++) {
