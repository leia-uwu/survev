...
import { ChestDef, HealDef, HelmetDef, LootDef, MeleeDef, OutfitDef, RoleDef, ThrowableDef } from '../../../shared/defs/objectsTypings';
import { Action, Anim, GameConfig, HasteType, WeaponSlot, Input } from "../../../shared/gameConfig";
import { PlayerInfo, getPlayerStatusUpdateRate } from "../../../shared/msgs/updateMsg";
import { coldet } from "../../../shared/utils/coldet";
import { collider } from "../../../shared/utils/collider";
import { collisionHelpers } from "../../../shared/utils/collisionHelpers";
import { math } from "../../../shared/utils/math";
import { ObjectData, ObjectType } from "../../../shared/utils/objectSerializeFns";
import { util } from "../../../shared/utils/util";
import { Vec2, v2 } from "../../../shared/utils/v2";
import { GroupStatus, LocalDataWithDirty, PlayerStatus } from "../../clientTypes";
...
import { BackpackDef, BoostDef } from './../../../shared/defs/gameObjects/gearDefs';
...
import { River } from "../../../shared/utils/river";
import { SoundHandle } from "../createJS";

const submergeMaskScaleFactor = 0.1;

function perksEqual(a: { type: string }[], b: { type: string}[]) {
...
export abstract class AbstractObject {
    ...
    abstract init(): void;
    abstract free(): void;
    abstract updateData(data: ObjectData<ObjectType>, fullUpdate: boolean, isNew: boolean, ctx: Ctx): void;
}
...
export class Player implements AbstractObject {
    ...
    updateData(data: ObjectData<ObjectType.Player>, fullUpdate: boolean, isNew: boolean, ctx: Ctx) {
    ...
    update(dt: number, playerBarn: PlayerBarn, map: Map, audioManager: AudioManager, particleBarn: ParticleBarn, inputBinds: InputBinds, camera: Camera, renderer: Renderer, ui2Manager: UiManager2, activeId: number, preventInput: boolean, displayingStats: boolean, isSpectating: boolean) {
        ...
        if ( Object.keys(this.surface?.data).length ) console.log(this.surface)
        // SOUND
        ...
        if (isActivePlayer && (weapTypeDirty || this.lastSwapIdx != this.localData.curWeapIdx)) {
            ...
            const itemDef = GameObjectDefs[this.netData.activeWeapon] as GunDef | MeleeDef | ThrowableDef;
            if (itemDef.type == "melee" || itemDef.type == "throwable") {
                ...
                if (
                    ...
                    const lastWeapDef = GameObjectDefs[this.localData.weapons[lastWeapIdx].type] as GunDef;
                    if (
                        itemDef &&
                        lastWeapDef &&
    ...
    updateVisuals(playerBarn: PlayerBarn, map: Map) {
        ...
        // Feet
        const setFootSprite = function(sprite: PIXI.Sprite, tint: number, downed: boolean) {
        ...
        const R = GameObjectDefs[this.netData.activeWeapon] as GunDef | MeleeDef | ThrowableDef;
        if (R.type == "gun") {
        ...
        if (R.type == "throwable") {
            const K = function(e: PIXI.Sprite, t: {
                sprite: string;
                pos?: Vec2;
                scale?: number;
            }) {
    ...
    playActionStartEffect(isActivePlayer: boolean, particleBarn: ParticleBarn, audioManager: AudioManager) {
        // Play action sound
    ...
    updateActionEffect(isActivePlayer: boolean, playerInfo: PlayerInfo, particleBarn: ParticleBarn, audioManager: AudioManager) {
    ...
    selectIdlePose() {
        const curWeapDef = GameObjectDefs[this.netData.activeWeapon] as GunDef | MeleeDef | ThrowableDef;
        let idlePose = "fists";
        idlePose = this.downed
            ? "downed"
            // @ts-expect-error no way I'm touching this
        ...
        if (this.currentAnim() != Anim.None) {
            ...
            for (let _ = 0; _ < a.effects.length; _++) {
                ...
                if (x.time >= r && x.time < f) {
                    ...
                    this[x.fn].apply(this, [AnimCtx, x.args]);
                }
            }
            if (w) {
                this.playAnim(Anim.None, this.anim.seq);
    ...
    animThrowableParticles(animCtx: Partial<AnimCtx>, args: unknown) {
        if ((GameObjectDefs[this.netData.activeWeapon] as ThrowableDef).useThrowParticles) {
            // Pin
            ...
            for (
                ...
                if (
                    ...
                    if (
                        ...
                        const hitSound =
                            meleeDef.sound[args.playerHit as keyof typeof meleeDef.sound] ||
...
export class PlayerBarn {
    ...
    playerIds: number[] = [];
    teamInfo: Record<number, {
        teamId: number,
        playerIds: number[]
    }> = {};
    groupInfo: Record<number, {
        groupId: number,
        playerIds: number[]
    }> = {}; 
    ...
    update(dt: number, activeId: number, r: unknown, renderer: Renderer, particleBarn: ParticleBarn, camera: Camera, map: Map, inputBinds: InputBinds, audioManager: AudioManager, ui2Manager: UiManager2, preventInput: boolean, displayingStats: boolean, isSpectating: boolean) {
        // Update players
    ...
    updatePlayerStatus(teamId: number, playerStatus: { players: PlayerStatus[]} , factionMode: boolean) {
    ...
    getPlayerName(playerId: number, activePlayerId: number, truncateForKillfeed: boolean) {
    ...
    addDeathEffect(targetId: number, killerId: number, sourceType: unknown, audioManager: AudioManager, particleBarn: ParticleBarn) {
