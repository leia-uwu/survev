...
import { AbstractObject, Player, PlayerBarn } from "./player";
...
export class Obstacle implements AbstractObject {
    ...
    updateData(data: ObjectData<ObjectType.Obstacle>, fullUpdate: boolean, isNew: boolean, ctx: Ctx) {
        if (fullUpdate) {
            this.type = data.type;
    ...
    update(dt: number, map: Map, playerBarn: PlayerBarn, particleBarn: ParticleBarn, audioManager: AudioManager, activePlayer: Player, renderer: Renderer) {
        if (this.isButton) {
    ...
    render(camera: Camera, debug: unknown, layer: unknown) {
