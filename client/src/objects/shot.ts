...
import { BulletParams } from './../../../server/src/objects/bullet';
import { ParticleBarn } from "./particles";
import { PlayerBarn } from "./player";
...
export function createCasingParticle(weapType: string, casingAngle: number, casingSpeedMult: number, pos: Vec2, dir: Vec2, layer: number, zOrd: number, particleBarn: ParticleBarn) {
...
export class ShotBarn {
    ...
    update(dt: number, activePlayerId: number, playerBarn: PlayerBarn, particleBarn: ParticleBarn, audioManager: AudioManager) {
        for (let i = 0; i < this.shots.length; i++) {
