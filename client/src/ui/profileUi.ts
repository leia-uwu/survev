...
function createLoginOptions(
    ...
    const addLoginOption = function(method: string, linked: boolean, onClick: () => void) {
        const el = $("<div/>", {
...
export class ProfileUi {
        ...
        $("#modal-account-name-finish").click((t) => {
            ...
            this.account.setUsername(name, (error?: string) => {
                if (error) {
                    ...
                    const message = ERROR_CODE_TO_LOCALIZATION[error as keyof typeof ERROR_CODE_TO_LOCALIZATION] || ERROR_CODE_TO_LOCALIZATION.failed;
    ...
    onItemsUpdated(items: {status: number}[]) {
