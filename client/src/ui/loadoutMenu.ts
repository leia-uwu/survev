import { HealDef } from './../../../shared/defs/gameObjects/gearDefs';
import { OutfitDef } from './../../../shared/defs/gameObjects/outfitDefs';
import { CrosshairDef } from './../../../shared/defs/gameObjects/crosshairDefs';
...
import loadout, { Loadout } from "./loadouts";
import { Localization } from "./localization";
import { MenuModal } from "./menuModal";
import { LoadoutDisplay } from "./opponentDisplay";
import { EmoteCategory, EmoteDef } from "../../../shared/defs/gameObjects/emoteDefs";
import { BoostDef, MeleeDef, OutfitDef } from "../../../shared/defs/objectsTypings";
...
const sortTypes: Record<string, (a: unknown, b: unknown) => void> = {
...
export class LoadoutMenu {
    ...
    onShow() {
        ...
        for (let i = 0; i < this.items.length; i++) {
            ...
            // @ts-expect-error status undefined ~~
    ...
    updateSlotData(parent: JQuery<HTMLElement>, img: string, type: string) {
        ...
        const emoteDef = GameObjectDefs[type] as EmoteDef;
    ...
    selectCat(catIdx: number) {
        ...
        if (r != this.selectedCatIdx) {
            let category = this.categories[r];
            for (
                let i = this.localAckItems.length - 1;
                i >= 0;
                i--
            ) {
        ...
        if (device.browser == "edge") {
            if (category.loadoutType == "crosshair") {
                const disableElem = function(parentElem: JQuery<HTMLElement>, disableElem: JQuery<HTMLElement>) {
                    const height =
                        parentElem.height()! +
