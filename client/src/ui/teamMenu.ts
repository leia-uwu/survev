import { Explosion } from './../../../server/src/objects/explosion';
import $ from "jquery";
...
export class TeamMenu {
    ...
    connect(create: boolean, roomUrl: string) {
        if (!this.active || roomUrl !== this.roomData.roomUrl) {
            ...
            const url = `ws://${roomHost}:3000/team_v2`;
            this.active = true;
            this.joined = false;
            this.create = create;
    ...
    refreshUi() {
        const setButtonState = function (el: JQuery<HTMLElement>, selected: boolean, enabled: boolean) {
            el.removeClass(
        ...
        if (this.joined) {
            ...
            for (
                ...
                if (this.editingName && playerStatus.self) {
                    ...
                    const m = () => {
                        const name = helpers.sanitizeNameInput(
                            n?.val() as string
                        );
