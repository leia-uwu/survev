...
import { RoleDef, RoleDefs } from "../../../shared/defs/gameObjects/roleDefs";
import { MapDef } from '../../../shared/defs/mapDefs';
import { GameConfig } from "../../../shared/gameConfig";
import { PlayerStatsMsg } from '../../../shared/msgs/playerStatsMsg';
import { MapIndicator } from '../../../shared/msgs/updateMsg';
...
import { Gas, GasRenderer, GasSafeZoneRenderer } from "../gas";
import { helpers } from "../helpers";
import "../inputBinds";
import { Map } from '../map';
import { MapIndicatorBarn } from "../objects/mapIndicator";
import { MapSprite, MapSpriteBarn } from "../objects/mapSprite";
...
import { PlayerStatus } from "../../clientTypes";
...
export class UiManager {
    ...
    display: {
        gas: PIXI.Graphics | PIXI.Sprite | null;
    ...
    update(dt: number, player: Player, map: Map, gas: Gas, i: unknown, playerBarn: PlayerBarn, camera: Camera, teamMode: number, factionMode: boolean) {
    ...
    updatePlayerMapSprites(dt: unknown, activePlayer: Player, playerBarn: PlayerBarn, map: Map) {
        const activePlayerInfo = playerBarn.getPlayerInfo(activePlayer.__id);

        let spriteIdx = 0;
        const addSprite = (pos: Vec2, scale: number, alpha: number, visible: boolean, zOrder: number, texture: string, tint: number) => {
    ...
    createPing(pingType: string, pos: Vec2, playerId: number, activePlayerId: number, playerBarn: PlayerBarn, factionMode: unknown) {
    ...
    /* not used? */
    updateMapSprite(mapSprite: any, sprite: PIXI.Sprite, spriteVisible: boolean, dt: number) {
    ...
    getOverviewElems(teamMode: number, teamRank: number, teamKills: number, factionMode: boolean) {
        if (factionMode) {
    ...
    showStats(playerStats: PlayerStatsMsg["playerStats"][], teamId: number, teamRank: number, winningTeamId: number, gameOver: boolean, localTeamId: number, teamMode: number, spectating: boolean, playerBarn: PlayerBarn, audioManager: unknown, map: Map, ui2: unknown) {
        ...
        if (!spectating || teamId == localTeamId || gameOver) {
            ...
            const T = function(e: string, t: number) {
                return $("<div/>", {
                    class: "ui-stats-info"
                })
                    .append(
    ...
    setSpectateTarget(targetId: number, localId: number, teamMode: number, playerBarn: PlayerBarn) {
    ...
    setLocalStats(stats: PlayerStatsMsg["playerStats"]) {
        ...
        for (const k in displayStats) {
            if (displayStats.hasOwnProperty(k)) {
                ...
                const stat = k == "timeAlive" ? humanizeTime(stats[k]) : stats[k as keyof typeof displayStats];
    ...
    updateKillLeader(playerId: number, playerName: string, kills: number, gameMode: MapDef["gameMode"]) {
...
updateSpectatorCountDisplay(dirty: boolean) {
    ...
    displayGasAnnouncement(type, timeLeft: number) {
        let message = "";
        switch (type) {
    ...
    render(playerPos: Vec2, gas: Gas, camera: unknown, map: Map, planeBarn: PlaneBarn, debug: unknown) {
        // Gas
    ...
    updateHealthBar(innerWidth: number, selectorInner: JQuery<HTMLElement>, selectorDepleted: JQuery<HTMLElement> | null, status: {
        health: number;
        dead: boolean;
        downed: boolean;
    }) {
    ...
    updateTeam(slotIdx: number, name: string, health: number, status: Partial<PlayerStatus>, playerId: number, o: unknown, s: unknown) {
    ...
    redraw(camera: Camera) {
        ...
        if (this.bigmapDisplayed) {
            ...
            let scaleCss: Record<number | string, string> = {
