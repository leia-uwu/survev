...
const loadout = {
    ...
    validate: function(userLoadout: Loadout) {
        ...
        const validatedLoadout: Loadout = {
            ...
            crosshair: {
                type: getGameType("crosshair", mergedLoadout.crosshair.type, "crosshair_default") as Crosshair["type"],
                color: (parseInt(mergedLoadout.crosshair.color as unknown as string) || 16777215),
                size: getFloat(mergedLoadout.crosshair.size  as unknown as string, 1).toFixed(2) as unknown as number,
                stroke: getFloat(mergedLoadout.crosshair.stroke  as unknown as string, 0).toFixed(2) as unknown as number
            },
            emotes: [] as string[]
        };
    ...
    */
    ...
    getUserAvailableItems: function(heroItems: unknown[]) {
        const items = [];
        // Add default items
        const unlockDefaultDef = GameObjectDefs.unlock_default as typeof UnlockDefs["unlock_default"];
        for (
            let i = 0;
            i < unlockDefaultDef.unlocks.length;
            i++
        ) {
